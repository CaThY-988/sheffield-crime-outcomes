...
def load_stop_search_reason_trends() -> pd.DataFrame:
    ...
    df["stop_search_count"] = pd.to_numeric(df["stop_search_count"], errors="coerce")
    df["pct_of_month_total"] = pd.to_numeric(df["pct_of_month_total"], errors="coerce")
...
def load_stop_search_outcome_mix() -> pd.DataFrame:
    ...
    df["stop_search_count"] = pd.to_numeric(df["stop_search_count"], errors="coerce")
    df["pct_within_object_of_search"] = pd.to_numeric(
        df["pct_within_object_of_search"], errors="coerce"
    )
    ...
    if stop_search_trends_df.empty:
        ...
        if trend_chart_df.empty:
            ...
            fig_stop_search_trend.update_layout(
                xaxis_title="Stop and search month",
